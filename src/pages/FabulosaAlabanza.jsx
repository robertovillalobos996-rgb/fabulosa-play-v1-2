...
const FabulosaAlabanza = () => {
  ...
  const hideTimeout = useRef(null); // NUEVO: Para ocultar controles
            ...
            <div ref={playerWrapperRef} onMouseMove={resetControlsTimer} onClick={resetControlsTimer} className="relative aspect-video w-full bg-black rounded-[2.5rem] overflow-hidden border-2 border-slate-800 shadow-2xl group cursor-pointer">
                ...
                <div className={`absolute bottom-0 w-full p-6 md:p-10 flex justify-between items-end bg-gradient-to-t from-black via-black/40 to-transparent transition-all duration-500 z-50 ${!showControls ? 'opacity-0 pointer-events-none' : 'opacity-100 translate-y-0'}`}>
                    <div className="flex gap-4">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setIsMuted(!isMuted); isMuted ? ytPlayerRef.current.unMute() : ytPlayerRef.current.mute(); }} className="p-4 bg-white/10 backdrop-blur-2xl border border-white/10 rounded-full hover:bg-white/20 transition-all">
                            {isMuted ? <VolumeX size={24}/> : <Volume2 size={24}/>}
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); continuarProgramacion(); }} className="p-4 bg-white/5 backdrop-blur-2xl border border-white/5 rounded-full hover:text-amber-500 transition-all">
                            <SkipForward size={24}/>
                        </button>
                    </div>
                    <div className="flex items-center gap-4">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); toggleFullscreen(); }} className="p-3 bg-white/5 rounded-full hover:bg-white/10 transition-all text-slate-400">
                            <Maximize size={20}/>
                        </button>
                    </div>
                </div>
