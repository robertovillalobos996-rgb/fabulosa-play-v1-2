...
const FabulositoKids = () => {
    const [view, setView] = useState("INTRO"); // INTRO -> HOME -> WATCH
    ...
    // 🔄 BUSCADOR CON ROTACIÓN DE LLAVES
    ...
    return (
        <div className="min-h-screen w-full bg-[#0014CC] font-sans overflow-hidden text-white">
                ...
                {view === "HOME" && (
                    <motion.div key="home" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="p-4 md:p-8 flex flex-col h-screen">
                        ...
                        <div className="flex justify-between items-center mb-6">
                            <motion.img whileHover={{ scale: 1.1, rotate: 5 }} src={LOGO_KIDS} className="h-16 md:h-24 drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]" />
                        ...
                        <div className="flex gap-4 md:gap-8 overflow-x-auto pb-6 no-scrollbar">
                            {CATEGORIAS.map(cat => (
                                <motion.button
                                    ...
                                    className={`${cat.color} ${activeCat === cat.id ? 'ring-8 ring-white shadow-[0_0_30px_white]' : 'opacity-80'} min-w-[150px] md:min-w-[250px] h-20 md:h-32 rounded-[2rem] flex flex-col items-center justify-center font-black italic tracking-tighter border-4 border-white transition-all`}
                                >
                                    {cat.icon}
                        ...
                        <div className="flex-1 overflow-y-auto grid grid-cols-2 md:grid-cols-4 gap-6 pr-2">
                            {videos.map((vid, index) => (
                                ...
                                >
                                    ...
                                    <p className="mt-3 font-bold text-xs md:text-sm line-clamp-2 text-center uppercase tracking-tight">{vid.snippet.title}</p>
                                </motion.div>
                            ))}
                        </div>
                    </motion.div>
                    ...
                    <motion.div key="watch" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 bg-black flex flex-col">
                        <div className="p-4 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent">
                            <button 
                                ...
                                className="bg-yellow-400 text-black px-8 py-3 rounded-full font-black uppercase italic hover:scale-110 transition-transform border-4 border-white"
                            >
                                🏠 VOLVER AL MUNDO
                            </button>
            ...
            {/* DECORACIONES FLOTANTES (ESTILO DISCOVERY KIDS) */}
            <div className="fixed -bottom-10 -left-10 w-64 h-64 bg-cyan-400/20 blur-[100px] rounded-full pointer-events-none" />
            <div className="fixed top-1/2 -right-10 w-48 h-48 bg-pink-500/20 blur-[80px] rounded-full pointer-events-none" />
        </div>
    );
};
