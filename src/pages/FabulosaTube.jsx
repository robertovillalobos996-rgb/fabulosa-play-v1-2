                ...
                <nav className="h-14 px-4 flex items-center justify-between bg-[#0f0f0f] sticky top-0 z-50 border-b border-[#272727]">
                    ...
                    <form onSubmit={handleSearch} className="flex-1 max-w-2xl mx-4 sm:mx-10 flex items-center">
                        <div className="flex w-full">
                            <input type="text" placeholder="Buscar en Fabulosa Play..." className="w-full bg-[#121212] border border-[#303030] rounded-l-full py-2 px-4 sm:px-6 text-white focus:outline-none focus:border-[#00b4d8]" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                            <button type="submit" className="px-4 sm:px-6 bg-[#222222] border border-l-0 border-[#303030] rounded-r-full hover:bg-[#303030]"><Search size={20} /></button>
                        </div>
                    </form>
                    ...
                    <div className="flex-1">
                        <div className="relative w-full aspect-video bg-black rounded-xl overflow-hidden shadow-2xl focus:ring-4 focus:ring-[#00b4d8] outline-none" tabIndex={0}>
                            {adState.isPlaying ? (
                                <div className="absolute inset-0 z-50 bg-black flex items-center justify-center">
                                    ...
                                    {/* 🔥 REPRODUCTOR DE ANUNCIOS BLINDADO 🔥 */}
                                    <video 
                                        key={currentAdUrl} 
                                        autoPlay 
                                        playsInline
                                        ...
                                        onEnded={skipAd} 
                                        onError={() => {
                                            console.warn("Falla en el video del anuncio, saltando...", currentAdUrl);
                                            skipAd();
                                        }}
                        ...
                        <div className="flex items-center justify-between mt-3 border-b border-[#3f3f3f] pb-4">
                            ...
                            <div className="flex gap-2">
                                <button className="px-4 py-2 bg-[#272727] hover:bg-[#3f3f3f] rounded-full text-sm font-bold flex items-center gap-2"><ThumbsUp size={18} /> Me Gusta</button>
    ...
    return (
        <div className="min-h-screen bg-[#0f0f0f] text-white font-sans flex flex-col overflow-hidden">
            <header className="h-14 px-4 flex items-center justify-between sticky top-0 bg-[#0f0f0f] z-50">
                ...
                <form onSubmit={handleSearch} className="flex-1 max-w-2xl mx-4 sm:mx-10 flex items-center">
                    <div className="flex w-full"><input type="text" placeholder="Buscar películas, música, directos..." className="w-full bg-[#121212] border border-[#303030] rounded-l-full py-2 px-4 sm:px-6 text-white focus:outline-none focus:border-[#00b4d8] transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /><button type="submit" className="px-4 sm:px-6 bg-[#222222] border border-l-0 border-[#303030] rounded-r-full hover:bg-[#303030]"><Search size={20} /></button></div>
                </form>
            ...
            <div className="flex flex-1 h-[calc(100vh-56px)]">
                ...
                <main className="flex-1 overflow-y-auto bg-[#0f0f0f] p-4 sm:p-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    <div className="flex gap-3 overflow-x-auto pb-4 sticky top-0 bg-[#0f0f0f] z-40 py-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                        {BUBBLE_TAGS.map((tag, i) => (<button key={i} onClick={() => { setSearchTerm(tag); document.querySelector('form').dispatchEvent(new Event('submit', { cancelable: true, bubbles: true })); }} className={`px-4 py-1.5 rounded-lg text-sm font-bold whitespace-nowrap transition-colors outline-none focus:ring-2 focus:ring-[#00b4d8] ${i === 0 ? 'bg-white text-black' : 'bg-[#272727] hover:bg-[#3f3f3f] text-white'}`}>{tag}</button>))}
                    </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-x-4 gap-y-10 pb-20 pt-4">
                            <div className="flex flex-col cursor-pointer focus:ring-4 focus:ring-[#00b4d8] outline-none rounded-xl tabIndex={0}">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <div><h3 className="text-white font-bold text-sm line-clamp-2">Contenido Patrocinado</h3><p className="text-[#aaa] text-xs mt-1">Fabulosa Play Premium</p></div>
                                </div>
                            </div>
